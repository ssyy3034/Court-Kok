...
- **문제점 (Challenge)**: 처음에는 익숙한 `Session` 기반으로 인증을 구현했습니다. 하지만 실시간 동기화를 위해 `WebSocket`을 도입하고 나니 문제가 생겼습니다. 웹소켓은 일반적인 HTTP 요청과 달라서, 세션 정보를 담은 쿠키가 제대로 전달되지 않는 경우가 많았습니다. 한마디로, 서버가 사용자를 알아보지 못하는 상황이 벌어진 거죠.

- **해결방안 (Solution)**: `WebSocket` 통신에는 매번 연결마다 인증 정보를 담아 보내는 방식이 더 적합하다는 것을 알게 되었습니다. 그래서 서버가 상태를 기억할 필요가 없는(Stateless) `JWT`로 전환했습니다. 사용자 스스로가 "나 인증된 사람이야"라는 토큰을 들고 다니는 방식이라, 웹소켓 연결이 새로 맺어질 때마다 이 토큰만 확인하면 됐습니다. 이 경험을 통해 **기술을 선택할 때는 개별 기능뿐만 아니라, 함께 사용하는 다른 기술과의 궁합을 반드시 고려해야 한다는 점**을 깨달을 수 있었습니다.
